...
inline void Spline1D<T>::splineIt(const bl::Array<double,1>& om, T df0, T dfN)
{
  ...
  if (df0==std::numeric_limits<double>::max() || dfN==std::numeric_limits<double>::max()){
    ...
    double df1 = (f(1)-f(0))/(om(1)-om(0));
    double x2 = 0.5*(om(2)+om(1));
    double df2 = (f(2)-f(1))/(om(2)-om(1));
...
public:
  ...
  bl::Array<double,1> dxi; // 1/(x_{j+1}-x_j)
public:
  explicit Spline2D(){};
...
inline T Spline2D<T>::operator()(const intpar& ipx, const intpar& ipy) const
{
  ...
  // Now that we have four values at ipy, we can interpolate through these four points a cubic interpolation (on equidistant mesh).
  // cubic interpolation on equdistant mesh
  int j = ipx.i;
  if (j>0 && j<N0-2){
