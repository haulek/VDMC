...
def Run0(QMC_filename,beta,Debug=False):
    ...
    for norder in range(start_order,MaxOrder+1):
        for Corder in range(norder): #[0]+range(2,norder):
            ...
            QMC = load(filename)
            Nthbin, Nkbin = shape(QMC) # theta-binnibg, k-binning
            ...
            QMCs = sum(QMC,axis=0) # sun over theta components because q=0.
            gm = (Rc00[:,0].real + 1.0)*dh_k[:]*dth  # vertex*dk*dh
            dPQ = dot(QMCs,gm)*beta
            PQw += dPQ
